...
var t0 = Date.now();
function log(str) {
	Message.log(`${Math.floor(Date.now() - t0)}: ` + str);
}
...
function createWebRtcOffer() {
	if (webRtcPlayerObj) {
		Message.log('Creating offer');
		showTextOverlay('Starting connection to server, please wait');
		webRtcPlayerObj.createOffer();
	} else {
		Message.log('WebRTC player not setup, cannot create offer');
...
function setupWebRtcPlayer(htmlElement, config) {
	Message.func('setupWebRtcPlayer')
	...
	webRtcPlayerObj.onWebRtcOffer = function (offer) {
		Message.func('webRtcPlayerObj.onWebRtcOffer')

		if (ws && ws.readyState === WS_OPEN_STATE) {
			let offerStr = JSON.stringify(offer);
			Message.log(`-> SS: offer:\n${offerStr}`);
			ws.send(offerStr);
		}
	};

	webRtcPlayerObj.onWebRtcCandidate = function (candidate) {
		Message.func('webRtcPlayerObj.onWebRtcCandidate')

		if (ws && ws.readyState === WS_OPEN_STATE) {
			Message.log(`-> SS: iceCandidate\n${JSON.stringify(candidate, undefined, 4)}`);
			ws.send(JSON.stringify({ type: 'iceCandidate', candidate: candidate }));
		}
	};

	webRtcPlayerObj.onVideoInitialised = function () {
		Message.func('webRtcPlayerObj.onVideoInitialised')
	...
	webRtcPlayerObj.onDataChannelConnected = function () {
		Message.func('webRtcPlayerObj.onDataChannelConnected')
	...
	webRtcPlayerObj.onDataChannelMessage = function (data) {
		...
		if (freezeFrame.receiving) {
			...
			if (freezeFrame.jpeg.length === freezeFrame.size) {
				...
				Message.log(`received complete freeze frame ${freezeFrame.size}`);
				showFreezeFrame();
			} else if (freezeFrame.jpeg.length > freezeFrame.size) {
				Message.error(`received bigger freeze frame than advertised: ${freezeFrame.jpeg.length}/${freezeFrame.size}`);
				freezeFrame.jpeg = undefined;
				freezeFrame.receiving = false;
			} else {
				Message.log(`received next chunk (${view.length} bytes) of freeze frame: ${freezeFrame.jpeg.length}/${freezeFrame.size}`);
			}
		} else if (view[0] === ToClientMessageType.QualityControlOwnership) {
			let ownership = view[1] === 0 ? false : true;
			Message.log("Received quality controller message, will control quality: " + ownership);
			...
			if (freezeFrame.jpeg.length < freezeFrame.size) {
				Message.log(`received first chunk of freeze frame: ${freezeFrame.jpeg.length}/${freezeFrame.size}`);
				freezeFrame.receiving = true;
			} else {
				Message.log(`received complete freeze frame: ${freezeFrame.jpeg.length}/${freezeFrame.size}`);
				showFreezeFrame();
			}
			...
			//Message.log(`received VideoEncoderAvgQP ${VideoEncoderQP}`);
		} else if(view[0] == ToClientMessageType.LatencyTest){
			let latencyTimingsAsString = new TextDecoder("utf-16").decode(data.slice(1));
			Message.log("Got latency timings from UE.")
			Message.log(latencyTimingsAsString);
			...
			Message.error(`unrecognized data received, packet ID ${view[0]}`);
		}
	};
...
function onWebRtcAnswer(webRTCData) {
	...
	{
		Message.func('latencyTestTimings.latencyTestTimings.OnAllLatencyTimingsReady')
...
function resizePlayerStyleToFillWindow(playerElement) {
	Message.func('resizePlayerStyleToFillWindow')
...
function resizePlayerStyleToActualSize(playerElement) {
	Message.func('resizePlayerStyleToActualSize')
...
function resizePlayerStyleToArbitrarySize(playerElement) {
	Message.func('resizePlayerStyleToArbitrarySize')
...
function setupFreezeFrameOverlay() {
	Message.func('setupFreezeFrameOverlay')
...
function showFreezeFrameOverlay() {
	Message.func('showFreezeFrameOverlay')
...
function invalidateFreezeFrameOverlay() {
	Message.func('invalidateFreezeFrameOverlay')

	freezeFrameOverlay.style.display = 'none';
	freezeFrame.valid = false;
}
function resizeFreezeFrameOverlay() {
	Message.func('resizeFreezeFrameOverlay')
...
function resizePlayerStyle(event) {
	Message.func('resizePlayerStyle')
...
function setupMouseAndFreezeFrame(playerElement) {
	Message.func('setupMouseAndFreezeFrame')
...
function updateVideoStreamSize() {
	Message.func('updateVideoStreamSize')
	...
	if (now - lastTimeResized > 1000) {
		...
		Message.log(descriptor);
		lastTimeResized = new Date().getTime();
	}
	else {
		Message.log('Resizing too often - skipping');
...
function onOrientationChange(event) {
	Message.func('onOrientationChange')
...
function emitUIInteraction(descriptor) {
	Message.func('emitUIInteraction', descriptor)
...
function setupNormalizeAndQuantize() {
	Message.func('setupNormalizeAndQuantize')
	...
	if (playerElement && videoElement.length > 0) {
		...
		if (playerAspectRatio > videoAspectRatio) {
			if (print_inputs) {
				Message.log('Setup Normalize and Quantize for playerAspectRatio > videoAspectRatio');
			}
...
function emitMouseMove(x, y, deltaX, deltaY) {
	...
	if (print_inputs) {
		Message.log(`x: ${x}, y:${y}, dX: ${deltaX}, dY: ${deltaY}`);
	}
...
function emitMouseDown(button, x, y) {
	...
	if (print_inputs) {
		Message.log(`mouse button ${button} down at (${x}, ${y})`);
	}
	let coord = normalizeAndQuantizeUnsigned(x, y);
...
function emitMouseUp(button, x, y) {
	...
	if (print_inputs) {
		Message.log(`mouse button ${button} up at (${x}, ${y})`);
	}
	let coord = normalizeAndQuantizeUnsigned(x, y);
	var Data = new DataView(new ArrayBuffer(6));
...
function emitMouseWheel(delta, x, y) {
	...
	if (print_inputs) {
		Message.log(`mouse wheel with delta ${delta} at (${x}, ${y})`);
	}
	let coord = normalizeAndQuantizeUnsigned(x, y);
...
function createOnScreenKeyboardHelpers(htmlElement) {
	Message.func('createOnScreenKeyboardHelpers')
...
function showOnScreenKeyboard(command) {
	Message.func('showOnScreenKeyboard')
...
function registerLockedMouseEvents(playerElement) {
	...
	function lockStateChange() {
		...
		if (document.pointerLockElement === playerElement ||
			...
			Message.log('Pointer locked');
			document.addEventListener("mousemove", updatePosition, false);
		} else {
			Message.log('The pointer lock status is now unlocked');
...
function registerHoveringMouseEvents(playerElement) {
	Message.func('registerHoveringMouseEvents')
...
function registerTouchEvents(playerElement) {
	Message.func('registerTouchEvents')
	...
	function rememberTouch(touch) {
		Message.func('rememberTouch')

		let finger = fingers.pop();
		if (finger === undefined) {
			Message.log('exhausted touch indentifiers');
		}
		fingerIds[touch.identifier] = finger;
	}

	function forgetTouch(touch) {
		Message.func('forgetTouch')
	...
	function emitTouchData(type, touches) {
		...
		for (let t = 0; t < touches.length; t++) {
			...
			if (print_inputs) {
				Message.log(`F${fingerIds[touch.identifier]}=(${x}, ${y})`);
			}
			let coord = normalizeAndQuantizeUnsigned(x, y);
...
function isKeyCodeBrowserKey(keyCode) {
	Message.func('isKeyCodeBrowserKey')
...
function registerKeyboardEvents() {
	Message.func('registerKeyboardEvents')

	document.onkeydown = function (e) {
		if (print_inputs) {
			Message.log(`key down ${e.keyCode}, repeat = ${e.repeat}`);
		}
	...
	document.onkeypress = function (e) {
		if (print_inputs) {
			Message.log(`key press ${e.charCode}`);
		}
...
function onExpandOverlay_Click(/* e */) {
	Message.func('onExpandOverlay_Click')
...
function connect() {
	...
	ws.onmessage = function (event) {
		Message.func('ws.onmessage')

		Message.log(`<- SS: ${event.data}`);
		...
		if (msg.type === 'config') {
			...
			Message.log(`invalid SS message type: ${msg.type}`);
		}
	};

	ws.onerror = function (event) {
		Message.func('ws.onerror')

		Message.log(`WS error: ${JSON.stringify(event)}`);

		APP.ws.onerror(event.code, event.reason)
	};

	ws.onclose = function (event) {
		Message.func('ws.onclose')
		APP.ws.onclose(event.code, event.reason)

		Message.log(`WS closed: ${JSON.stringify(event.code)} - ${event.reason}`);
		ws = undefined;
...
function onConfig(config) {
	...
	switch (inputOptions.controlScheme) {
		...
		case ControlSchemeType.LockedMouse:
			...
			break;
		default:
			Message.log(`ERROR: Unknown control scheme ${inputOptions.controlScheme}`);
			registerLockedMouseEvents(playerElement);
			break;
	}
}
...
import {BaseCommands, Message} from './middleware/'

class Controller extends BaseCommands {
	constructor(...args) {
		super(...args)
	...
	get callbacks() {
		...
		const set = () => {
			...
			// addResponseEventListener(event_handler_key, (payload_str) => {
			//
			// 	try {
		...
		const remove = () => {
			...
			try {
				...
				Message.error('removeResponseEventListener', err)
			}
		}
	...
	onVideoInitialised() {
		...
		Message.func('listener', 'onVideoInitialised')
	...
	get events() {
		...
		return {mouseDown, mouseDownMove};
	}

	get ws() {
		const onmessage = (msg, data) => {

			Message.error('onmessage', {msg, data});
		...
		const onerror = (code, reason) => {
			Message.error('onerror', {code, reason})
			this.loadedTimes = 0
			this.actions.onError({code, reason})
		}

		const onclose = (code, reason) => {
			Message.error('onclose', {code, reason})
			this.loadedTimes = 0
			this.actions.onClose({code, reason})
	...
	onDataChannelMessage([code, value]) {
...
class AppClass extends Controller {
	...
	get socketUrl() {
		const ws_config = window.PixelStreamingConfig
		const parsedUrl = url.parse(ws_config.host)
		...
		Message.log(`Connection to:`, socketUrl);
	...
	start({host, port=80, warnTimeout=120, closeTimeout=10, lockMouse=false, autoPlay=true} = {}) {

		// if(this.redirect()) return ;

		if(host) {
			// host = host || this.config.stream.host
			window.PixelStreamingConfig = {host, port}
			Message.warn('config', window.PixelStreamingConfig);
		}
		...
		afk = {
			...
			warnTimeout, // The time to elapse before warning the user they are inactive.
	...
	emit({type, value, verification_id}) {
		Message.warn('emit', {type, value, verification_id});
		emitUIInteraction({type, value, verification_id})
	}
